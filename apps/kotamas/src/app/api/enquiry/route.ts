// File: src/app/api/enquiry/route.ts — KOTAMAS
import { enquirySchema } from "@/lib/validations";

const MODULE_SLUG = "kotamas";

/**
 * POST /api/enquiry
 * KOTAMAS Form submission endpoint.
...
export async function POST(request: Request) {
  try {
    ...
    try {
      for (const item of submissionsToPost) {
        ...
        if (!res.ok) {
          ...
          console.error(`[KOTAMAS Enquiry API] Error response from Admin API (${res.status}):`, errText);
        }
      }
    } catch (adminErr) {
      console.warn("[KOTAMAS Enquiry API] Warning dispatching to Admin Dashboard:", adminErr);
    }
    ...
    console.error("[KOTAMAS Enquiry API] Error:", error);
    return Response.json(
      {
        success: false,
