// File: src/app/api/enquiry/route.ts — KRTB
import { enquirySchema } from "@/lib/validations";

const MODULE_SLUG = "krtb";

/**
 * POST /api/enquiry
 * KRTB Form submission endpoint.
...
export async function POST(request: Request) {
  try {
    ...
    try {
      for (const item of submissionsToPost) {
        ...
        if (!res.ok) {
          ...
          console.error(`[KRTB Enquiry API] Error response from Admin API (${res.status}):`, errText);
        }
      }
    } catch (adminErr) {
      console.warn("[KRTB Enquiry API] Warning dispatching to Admin Dashboard:", adminErr);
    }
    ...
    console.error("[KRTB Enquiry API] Error:", error);
    return Response.json(
      {
        success: false,
