// File: src/app/api/enquiry/route.ts — KOPETRO
import { enquirySchema } from "@/lib/validations";

const MODULE_SLUG = "kopetro";

/**
 * POST /api/enquiry
 * KOPETRO Form submission endpoint.
...
export async function POST(request: Request) {
  try {
    ...
    try {
      for (const item of submissionsToPost) {
        ...
        if (!res.ok) {
          ...
          console.error(`[KOPETRO Enquiry API] Error response from Admin API (${res.status}):`, errText);
        }
      }
    } catch (adminErr) {
      console.warn("[KOPETRO Enquiry API] Warning dispatching to Admin Dashboard:", adminErr);
    }
    ...
    console.error("[KOPETRO Enquiry API] Error:", error);
    return Response.json(
      {
        success: false,
