    ...
    try {
      for (const item of submissionsToPost) {
        ...
        if (!res.ok) {
          ...
          console.error(`[KOHASiL Enquiry API] Error response from Admin API (${res.status}):`, errText);
        }
      }
    } catch (adminErr) {
      console.warn("[KOHASiL Enquiry API] Warning dispatching to Admin Dashboard:", adminErr);
    }
